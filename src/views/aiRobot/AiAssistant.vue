<template>
  <div class="ai-assistant-container">
    ...
    <div class="toolbar">
      <div class="left">
        <el-button size="small" @click="createNewSession" :disabled="isGenerating">
          新建会话
        </el-button>
        <el-button size="small" @click="clearHistory" :disabled="isGenerating">
          清空历史
        </el-button>
      </div>
      <div class="right">
        <el-button size="small" @click="handleRegenerate" :disabled="isGenerating || !canRegenerate">
          重新生成
        </el-button>
        <el-button size="small" type="danger" @click="handleStop" :disabled="!isGenerating">
          停止
        </el-button>
      </div>
    </div>
...
const canRegenerate = computed(() => {
  ...
  // 末尾是assistant或没有assistant都允许重新生成
  return true
})
...
async function generateAssistantReply(seedIndex?: number) {
  isGenerating.value = true
  abortController = new AbortController()

  // 追加一个空的assistant消息用于流式填充
  const assistantMsg: ChatMessage = { role: 'assistant', content: '' }
  messages.value.push(assistantMsg)
  scrollToBottom()

  try {
    // 如果有真实后端接口，这里替换为你的接口地址
...
function retryFrom(index: number) {
  if (isGenerating.value) return
  // 从这条之前的全部消息组成上下文，并重新生成
  const slice = messages.value.slice(0, index)
...
.typing .dot {
  width: 6px; height: 6px; border-radius: 50%; background: #909399; opacity: .6;
